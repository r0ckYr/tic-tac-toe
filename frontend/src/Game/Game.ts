...
export class TicBoard {
  ...
  public makeMove(player: string, position: number[]): GameBoard {
      // Check if the correct player plays
      if(this.chance!=player) {
        return { validMove: false, chance: this.chance, gameEnds: false, winner: "E", board: this.gameBoard };
      }
      ...
      if (this.checkWinner(player)) {
          return { validMove: true, chance: this.chance, gameEnds: true, winner: player, board: this.gameBoard };
      }

      // Check if the game is a draw
      const isBoardFull = this.gameBoard.every(row => row.every(cell => cell !== "E"));
      if (isBoardFull) {
          return { validMove: true, chance: this.chance, gameEnds: true, winner: "E", board: this.gameBoard };
      }

      // Game continues
      return { validMove: true, chance: this.chance, gameEnds: false, winner: "E", board: this.gameBoard };
  }
}